...
class HyparGeo(HasTraits):
    ...
    def __call__(self, X_arr):
        ...
        y = (yi) * self.ly
        z = zi * self.lz
        a = self.lx ** 2 / self.zminus
        b = self.ly ** 2 / self.zplus
        z += y ** 2 / b - x ** 2 / a
...
#        return self.shape_L1 + self.shape_L2
    ...
    #
    mats_eval = Property(Instance(MATS3DMicroplaneDamage),
...
#        mats_eval = MATS3DMicroplaneDamage(elastic_debug=True,
#                                E=self.E_c,
#                                nu=self.nu,
#                                #n_mp=30,
...
#                                phi_fn=self.phi_fn)
    ...
    hypar_geo = Property(Instance(HyparGeo), depends_on='+input')
    @cached_property
    def _get_hypar_geo(self):
        return HyparGeo()

    fe_grid = Property(Instance(FEGrid), depends_on='+ps_levels, +input')
    @cached_property
    def _get_fe_grid(self):
        fe_grid = FEGrid(coord_min=(-1, -1, -1),
                         coord_max=(1, 1, 1),
                         shape=(self.shape_x,
                                self.shape_y,
                                self.shape_z),
                         level=self.specmn_fe_level,
                         geo_transform=self.hypar_geo,
                         fets_eval=self.specmn_fets)
...
#                                                var = 'u', idx = 0, warp = True ),
    ...
    def _get_tloop(self):
        ...
        midnode_x = self.shape_x / 2
        midnode_y = self.shape_y / 2
        bc_fix_x = BCSlice(var='u', value=0, dims=[0], slice=fe_grid[0, 0, 0, 0, 0, 0])
        bc_fix_y = BCSlice(var='u', value=0, dims=[1], slice=fe_grid[midnode_x, midnode_y, 0, 0, 0, 0])
        bc_fix_z0 = BCSlice(var='u', value=0, dims=[2], slice=fe_grid[0, :, 0, 0, :, 0])
        bc_fix_z1 = BCSlice(var='u', value=0, dims=[2], slice=fe_grid[-1, :, 0, -1, :, 0])

        bc_list = [bc_fix_x,
                   bc_fix_y, bc_fix_z0, bc_fix_z1  ] + force_bc

        w_z = fe_grid[-1, -1, -1, -1, -1, -1].dofs[0, 0, 2]

        f_w_diagram = RTraceGraph(name='load - corner deflection',
                                           var_x='U_k', idx_x=w_z,
                                           var_y='time', idx_y=0,
                                           record_on='update')

        rtrace_list = [ f_w_diagram ] + self.rtrace_list
...
#                                age = 28 )
    ...
    hypar_geo = HyparGeo(ly=1, lx=5, zplus=0.4, zminus=0.7)
    # s_tex,z = 4.62 mm corresponds to: n_tex = 12; h = 60 mm
    #
    sim_model = SimHyparDB(ccs_unit_cell_key='FIL-10-09_2D-05-11_0.00462_all0',
