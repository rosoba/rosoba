...
#                                                        correction=True))
    ...
    aa = np.hstack((loading_dof_ps, np.unique(bc_slice_right.dofs[:, :, 0].flatten()), np.unique(bc_slice_down.dofs[:, :, 0].flatten())))
    print 'aa', aa
...
#              u_processor=avg_processor,
    ...
    tl = TLoop(tstepper=ts,
                tolerance=5.0e-5,
                KMAX=200,
                tline=TLine(min=0.0, step=.01, max=0.8))
