...
#     loading_slice = fe_grid[n_el_x - 1 , -1, 0, 0] #left down Node 0 [0,1]
    ...
    dof_linked_left = fe_grid[0, -1, 0, -1].dofs[0, 0, 1].flatten()[0]
    bc_link_left = BCDof(var='u', value=0., dof=dof_linked_left ,
                              link_dofs=[dof_ref],
                              link_coeffs=[1.0])  #


    dof_linked_right = fe_grid[-1, -1, 0, -1].dofs[0, 0, 1].flatten()[0]
    bc_link_right = BCDof(var='u', value=0., dof=dof_linked_right ,
                                  link_dofs=[dof_ref],
                                  link_coeffs=[1.0])


    dof_linked_list = fe_grid[-1, :, -1, -1].dofs[0, 0, 1].flatten()[0]
